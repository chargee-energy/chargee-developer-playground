...
import { lookupAddress, normalizeLookupInput } from '@/utils/addressLookup'
    ...
    <form onSubmit={submit} className="flex flex-col items-center gap-1 sm:items-start">
      <div className="flex h-9 items-center gap-1 rounded-full border border-beige-2 bg-white pl-3 pr-1">
        ...
        <input
          ...
          value={query}
          onChange={(e) => {
            // Strip stray whitespace from pasted values as they come in, so the
            // field shows exactly what will be looked up.
